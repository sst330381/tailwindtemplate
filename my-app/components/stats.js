function Page() {
  const desc = [
    { title: "8,000+", subtitle: "Creators on the platform" },
    { title: "3%", subtitle: "Flat platform fee" },
    { title: "99.9%", subtitle: "Uptime guarantee" },
    { title: "$70M", subtitle: "Paid out to creators" },
  ];
  return (
        ...
        <div>
          ...
          <dl className="mt-5 grid grid-cols-1 divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow md:grid-cols-4 md:divide-x md:divide-y-0">
            {desc.map(x=>(
              ...
              <dt className="text-base font-normal text-gray-900">{x.subtitle}</dt>
              <dd className="mt-1 flex items-baseline justify-between md:block lg:flex">
                <div className="flex items-baseline text-2xl font-semibold text-indigo-600">
                  {x.title}<span className="ml-2 text-sm font-medium text-gray-500"></span>
                </div>
